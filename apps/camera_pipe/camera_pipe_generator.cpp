...
public:
    ScheduleParam<LoopLevel> intermed_compute_at{"intermed_compute_at", "undefined"};
    ScheduleParam<LoopLevel> intermed_store_at{"intermed_store_at", "undefined"};
    ScheduleParam<LoopLevel> output_compute_at{"output_compute_at", "root"};
    ...
    void generate() {
        // assert(output_compute_at.defined());
    ...
    void schedule() {
        ...
        // TODO this is really awkward syntactically.
        LoopLevel ica = intermed_compute_at.defined() ? intermed_compute_at : LoopLevel(output, y);
        LoopLevel isa = intermed_store_at.defined() ? intermed_store_at : LoopLevel(output, y);
...
Func CameraPipe::build() {
    ...
    // If we 
    auto demosaiced = Demosaic::create(this);
    demosaiced.generate(deinterleaved);
    ...
    demosaiced
        .set_schedule_param("intermed_compute_at", LoopLevel(processed, yi))
        .set_schedule_param("intermed_store_at", LoopLevel(processed, yo))
        .set_schedule_param("output_compute_at", LoopLevel(processed, x))
        .schedule();
