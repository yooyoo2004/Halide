...
private:
    ...
    Expr simplify_to_true(Expr cond) {
        ...
        Expr relaxed_cond = and_condition_over_domain(cond, inner_loop_vars, bound_vars, &tight);
        if (!equal(relaxed_cond, cond)) {
            // There may be new Ands, Ors, Nots, etc
            debug(3) << "  Relaxed " << cond << " -> " << relaxed_cond << "\n";
...
class ContainsNonTrivialInnerLoop : public IRVisitor {
    using IRVisitor::visit;
...
bool contains_non_trivial_inner_loop(Stmt s) {
    ContainsNonTrivialInnerLoop b;
    s.accept(&b);
    return b.result;
}
