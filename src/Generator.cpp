...
private:
    ...
    void emit_generator_params_setters();
...
void StubEmitter::emit_generator_params_setters() {
    stream << indent() << "// set_generator_param methods\n";
    stream << indent() << "template <typename T>\n";
    stream << indent() << class_name << " &set_generator_param(const std::string &name, const T &value) {\n";
    indent_level++;
    stream << indent() << "(void) GeneratorStub::set_generator_param(name, value);\n";
    ...
    if (!v.empty()) {
        for (auto p : v) {
            ...
            stream << indent() << "return set_generator_param(\"" << p->name <<  "\", " << value << ");\n";
            indent_level--;
            stream << indent() << "}\n";
        }
        stream << "\n";
...
void StubEmitter::emit_schedule_params_setters() {
    ...
    for (auto *sp : schedule_params) {
        ...
        indent_level++;
        stream << indent() << sp->name() << ".set(value);\n";
        stream << indent() << "return *this;\n";
        indent_level--;
        stream << indent() << "}\n";
    }
...
void StubEmitter::emit() {
   ...
   struct OutputInfo {
    ...
    stream << indent() << "class " << class_name << " final : public Halide::GeneratorStub {\n";
